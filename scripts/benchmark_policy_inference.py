...
os.environ.setdefault("OPENPI_MANUAL_CUDAGRAPH", "1")  # capture+replay full sample_actions
os.environ.setdefault("AITER_PRESHUFFLE_WEIGHTS", "0")  # off for best e2e latency
os.environ.setdefault("OPENPI_EAGER_ATTN_USE_SDPA", "1")  # SDPA fast-path for KV-cache attention
os.environ.setdefault("OPENPI_FUSE_SIGLIP_QKV", "1")  # fuse SigLIP Q/K/V projections
os.environ.setdefault("OPENPI_ROUTE_FUSED_LINEAR_TO_AITER", "1")  # route fused projections to aiter
...
def main():
    ...
    # Pi0.5 mode: OPENPI_PI05=1
    config.pi05 = os.environ.get("OPENPI_PI05", "0") == "1"
    ...
    model_name = "pi0.5" if config.pi05 else "pi0"
    print(f"[openpi] model={model_name} action_horizon={config.action_horizon}", flush=True)
